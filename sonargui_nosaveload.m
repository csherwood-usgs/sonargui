function varargout = sonargui_nosaveload(varargin)
% SONARGUI_nosaveload MATLAB code for sonarGUI_nosaveload.fig
%      SONARGUI_nosaveload, by itself, creates a new SONARGUI_nosaveload or raises the existing
%      singleton*.
%
%      H = SONARGUI returns the handle to a new SONARGUI_nosaveload or the handle to
%      the existing singleton*.
%
%      SONARGUI_nosaveload('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SONARGUI_nosaveload.M with the given input arguments.
%
%      SONARGUI_nosaveload('Property','Value',...) creates a new SONARGUI_nosaveload or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before sonarGUI_nosaveload_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to sonarGUI_nosaveload_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help sonarGUI_nosaveload

% Last Modified by GUIDE v2.5 08-Nov-2012 09:05:24

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
   'gui_Singleton',  gui_Singleton, ...
   'gui_OpeningFcn', @sonargui_nosaveload_OpeningFcn, ...
   'gui_OutputFcn',  @sonargui_nosaveload_OutputFcn, ...
   'gui_LayoutFcn',  [] , ...
   'gui_Callback',   []);
...
% --- Executes just before sonargui_nosaveload is made visible.
function sonargui_nosaveload_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to sonargui_nosaveload (see VARARGIN)

% Choose default command line output for sonargui_nosaveload
handles.output = hObject;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Load the initial tripod and instrument geometries- 
%   you're only here the first time through the program
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
d2r = pi/180;
...
switch lower(experiment)
   case 'unh_flat'
      unh_geom
         fnames=dir('c:\home\data\unh\sonar_data\');
  ...
  case 'unh_tilted'
   ...
   case 'FI12'
      FI12_geom
        fnames=dir('c:\home\data\FI2012\sonar_post\Iris_az\');
        cellnames={fnames.name};  % put all the names in a cell array
        locs=strfind(cellnames,'.cdf'); %find the ones with cdf
         isntcdf=cellfun(@isempty,locs);   %the notCDFs will be []
         iscdf=find(isntcdf==0);           % get the indices
...
% so window can get raised using sonarguiFI.
if strcmp(get(hObject,'Visible'),'off')
   replot(handles);
end

% UIWAIT makes sonarguiFI wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = sonargui_nosaveload_OutputFcn(hObject, eventdata, handles)
% varargout  cell array for returning output args (see VARARGOUT);
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

disp ('in sonargui ouputFcn')
keyboard
% hObject on return is the same as handles.figure1
% handles.output is the same value as handles.figure1 and hObject
% get(gcbo) is [] here
...
disp ('in experiment callback')
experiment = get(handles.experiment_popupmenu6,'Value');
% these *_gemo programs read details about measuered positions of things on
% the tripod for each experiment

experiment=guihandles.experiment;
switch experiment
   case 1
      unh_geom
...
handles.expname=exname;
% put the new names in the handle
set(handles.azdata_popupmenu5,'String',azlist)
% then issue the callback to renerate whats displayed?
%  azdata_popupmenu5_Callback(hObject, eventdata, handles)
plotinfo=handles.plotinfo;
inst = 1;
handles.inst = 1;
handles.old_inst = 1;
% all this info must be saved in order to be available to other functions
% the old way is:
%   save sonar_output.mat inst instloc tripod cmpss plotinfo
% the more Matlab GUI way is with guidata(), which both saves and loads the
% conetnt of handles
guidata(hObject,handles); % Store the changes into the handles thing.
replot (handles)
...
disp('in azdata_popup5 Callback function')
old_az=get(handles.azdata_popupmenu5,'String')
...
switch experiment
   ...
   case 3
      exname='hatteras 2009'
   case 4
       disp('trying to put az files in the list')
      dpath='c:\home\data\FI2012\sonar_post\Iris_az\';
      exname='Fire Island 2012';
        fnames=dir(dpath);
        cellnames={fnames.name};  % put all the names in a cell array
...
% the files in the popup are from guide- they should be entered from the
% dirctory with the experiment data.
...
d2r = pi/180.;
r2d = 1/d2r;
% see what is in the azdata list
disp('in replot')
%h1=guidata(gcbo);
azlist=get(handles.azdata_popupmenu5,'String')
%reset the list of files appropriate to the experiment
%set(handles.azdata_popupmenu5,'String',azlist);
%load sonar_output.mat
%handles.tripod=tripod;
% Grab values from sliders and edit windows for whatever the handles.inst 
% is set to- 1=tripod, 2=adcp, etc based on what's in the _geom file
% after initial call has data from _geom, but all these get's the instlocs
% end up with 0's
instloc(handles.inst).xyz(1) = get(handles.easting_slider5,'Value');  % Height of transducer
instloc(handles.inst).xyz(2) = get(handles.northing_slider6,'Value');
instloc(handles.inst).xyz(3) = get(handles.height_slider,'Value');
instloc(handles.inst).pry(1) = get(handles.pitch_slider,'Value');
instloc(handles.inst).pry(2) = get(handles.roll_slider,'Value');
instloc(handles.inst).pry(3) = get(handles.yaw_slider,'Value');
...
use_compass = get(handles.usecompass_radiobutton8,'Value');
%  why can't this call be used outside of use_compass case below??
%   guidata(hObject,handles) % Store the changes.
...
hold off
keyboard
% first time thorough, there is no figure at this line- it is deawn at the
% close of this function, when it goes to sonargui_nosaveload_outputFcn
%
% after the initial figure creation
