function varargout = sonargui_nsl(varargin)
% SONARGUI_NSL MATLAB code for sonargui_nsl.fig
%      SONARGUI_NSL, by itself, creates a new SONARGUI_NSL or raises the existing
%      singleton*.
%
%      H = SONARGUI_NSL returns the handle to a new SONARGUI_NSL or the handle to
%      the existing singleton*.
%
%      SONARGUI_NSL('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in SONARGUI_NSL.M with the given input arguments.
%
%      SONARGUI_NSL('Property','Value',...) creates a new SONARGUI_NSL or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before sonargui_nsl_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to sonargui_nsl_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help sonargui_nsl

% Last Modified by GUIDE v2.5 13-Nov-2012 16:05:21

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
   'gui_Singleton',  gui_Singleton, ...
   'gui_OpeningFcn', @sonargui_nsl_OpeningFcn, ...
   'gui_OutputFcn',  @sonargui_nsl_OutputFcn, ...
   'gui_LayoutFcn',  [] , ...
   'gui_Callback',   []);
...
% --- Executes just before sonargui_nsl is made visible.
function sonargui_nsl_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to sonargui_nsl (see VARARGIN)

% Choose default command line output for sonargui_nsl
...
[ra,hdg]=pcoord(xa,ya);
% and write intot he boxes under "use this compass" button
str = num2str(hdg);
...
% so window can get raised using sonargui_nsl.
if strcmp(get(hObject,'Visible'),'off')
   replot(handles);
end

% UIWAIT makes sonargui_nsl wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = sonargui_nsl_OutputFcn(hObject, eventdata, handles)
...
where_from=gcbo;
if where_from == handles.experiment_popupmenu6
   inst=handles.inst;
else
...
% the tripod for eacdh experiment
switch experiment
   case 1
      unh_geom
...
handles.experiment=experiment;
handles.expname=exname;
% when reseting, we want to return to the tripod view initially
inst = 1;
handles.inst = 1;
handles.old_inst = 1;
replot (handles )
...
d2r = pi/180.;
r2d = 1/d2r;
% don't need to update it in replot, since it's updated in the callbacks
% load sonar_output.mat
% handles.tripod=tripod;
% Grab values from sliders and edit windows for whatever the inst is set to
...
% handles.instloc(handles.inst).xyz = instloc.xyz;
% handles.instloc(handles.inst).pry = instloc.pry;
%guidata(hObject,handles); % Store the changes.
