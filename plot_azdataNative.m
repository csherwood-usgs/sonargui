...
%beta = squeeze(hang(tidx,:,1:end-ntrim))+tilt; % pencil head angle, conv. to radiansbeta = d2r*beta
beta = squeeze(hang(:,:,tidx))+tilt; % pencil head angle, conv. to radiansbeta = d2r*beta;
...
if(1) % estimate profile ranges from maxima of returns
   for iAz=1:nang
     raw_img=squeeze(raw_image(:,50:end,iAz,tidx));
